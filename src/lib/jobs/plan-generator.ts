...
async function savePlansToDatabase(
  ...
  let failedPlansCount = 0;

  // ✅ Use transaction with increased timeout (30 seconds)
  await prisma.$transaction(
      ...
      for (const plan of plans) {
        try {
          ...
          if (!planData || typeof planData !== "object") {
            console.error(
              `Plan ${plan.planNumber} has invalid planData:`,
              planData
            );
            throw new Error(
              `خطة رقم ${plan.planNumber} لا تحتوي على بيانات صالحة`
            );
          }

          // ✅ Extract with fallbacks for different key formats
          const title =
            planData.title || planData.Title || `خطة علاجية ${plan.planNumber}`;
          const generalGoal =
            planData.generalGoal ||
            planData.general_goal ||
            planData.GeneralGoal ||
          ...
          if (!Array.isArray(stages) || stages.length === 0) {
            console.warn(
              `Plan ${plan.planNumber} has no stages, skipping stage/task creation`
            );
            savedPlansCount++;
            continue;
          }
          ...
          if (stagesToCreate.length === 0) {
            console.warn(
              `Plan ${plan.planNumber} has no valid stages to create`
            );
            savedPlansCount++;
            continue;
          }
          ...
          savedPlansCount++;
          console.log(
            `✅ Saved plan ${plan.planNumber}/4 with ${allTasksToCreate.length} tasks`
          );
        } catch (planError: any) {
    ...
    {
      maxWait: 30000, // 30 seconds - maximum time to wait to start the transaction
      timeout: 30000, // 30 seconds - maximum time for the transaction to complete
    }
  );

  // ✅ Update job progress OUTSIDE the transaction
  if (savedPlansCount > 0) {
    await prisma.planGenerationJob.update({
      where: { id: jobId },
      data: {
        progress: 95,
        currentStep: `Saved ${savedPlansCount} plans to database successfully`,
      },
    });
  }
